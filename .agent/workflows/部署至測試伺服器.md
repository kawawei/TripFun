...
2.  **Deploy to Test Server & Pull Latest Code**
    ...
    // turbo
    sshpass -p 'Ben01270127-' ssh -o StrictHostKeyChecking=no root@43.103.3.57 "cd /opt/project/TripFun && git fetch --all && git reset --hard origin/main && docker compose -f docker/prod/compose.yaml up -d --build --force-recreate --no-deps backend"

3.  **Check Health Status**
    Verify that the tripfun containers show a `(healthy)` or `Up` status.
    // turbo
    sshpass -p 'Ben01270127-' ssh -o StrictHostKeyChecking=no root@43.103.3.57 "docker ps --filter 'name=tripfun' --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'"
