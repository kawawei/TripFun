---
description: Deploy latest main branch to Test Server (43.103.3.57) for TripFun project
---

This workflow automates the deployment process to the test server. It updates the code and rebuilds the backend container.

**專案路徑**: `/opt/project/TripFun`
**Docker Compose 路徑**: `docker/prod/compose.yaml`
**容器前綴**: `tripfun-`
...
2.  **Deploy to Test Server & Pull Latest Code**
    Connect via SSH, pull latest code using `reset --hard` to avoid conflicts, and rebuild backend service.
    Note: `--env-file env/prod/.env` is required to inject DB/Redis credentials correctly.
    // turbo
    sshpass -p 'Ben01270127-' ssh -o StrictHostKeyChecking=no root@43.103.3.57 "cd /opt/project/TripFun && git fetch --all && git reset --hard origin/main && docker compose -f docker/prod/compose.yaml --env-file env/prod/.env up -d --build --force-recreate --no-deps backend"

3.  **Check Health Status**
    Verify that the tripfun containers show a `(healthy)` or `Up` status.
    // turbo
    sshpass -p 'Ben01270127-' ssh -o StrictHostKeyChecking=no root@43.103.3.57 "docker compose -f docker/prod/compose.yaml --env-file env/prod/.env ps --filter 'name=tripfun' --format 'table {{.Names}}\t{{.Status}}\t{{.Ports}}'"
