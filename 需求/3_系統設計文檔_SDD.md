...
### 12.1 測試伺服器端口規劃 (Test Server: 43.103.3.57)

> 已於 2026-04-24 查詢伺服器現有容器端口，以下為 TripFun 規劃使用之未占用端口：

**伺服器已占用端口（其他專案）：**

| 類型 | 已占用端口 |
| :--- | :--- |
| **前端 HTTP** | 80, 81, 82, 83, 84, 85, 86, 88 |
| **後端 API** | 8080, 8081, 8082, 8083, 8084, 8085, 8086 |
| **PostgreSQL** | 5432, 5433, 5434, 5435, 5436, 5437 |
| **Redis** | 6379, 6380, 6381, 6382, 6383, 6384 |

**TripFun 伺服器分配端口（避開以上衝突）：**

| 服務名稱 | 伺服器映射端口 | 備註 |
| :--- | :--- | :--- |
| **前端 (Flutter Web)** | **89** | Nginx 對外 HTTP |
| **後端 (NestJS API)** | **8087** | API 服務 |
| **PostgreSQL** | **5438** | 資料庫 |
| **Redis** | **6385** | 緩存服務 |
...
#### [PATCH] /trips/:id
*   **說明**: 修改行程資訊（如更換標題、日期、圖示色值、**旅客數量**等）。
*   **參數**: `id` (Trip UUID)
